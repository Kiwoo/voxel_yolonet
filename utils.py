...
def draw_lidar_box3d_on_birdview(birdview, boxes3d, scores, gt_boxes3d=np.array([]), 
    ...
    for box in corner_gt_boxes3d:
        if all(cfg.X_MIN < x < cfg.X_MAX for x in box[:, 0]) and all(cfg.Y_MIN < y < cfg.Y_MAX for y in box[:, 1]):
    ...
    for box in corner_boxes3d:
        if all(cfg.X_MIN < x < cfg.X_MAX for x in box[:, 0]) and all(cfg.Y_MIN < y < cfg.Y_MAX for y in box[:, 1]):
            ...
            # warn("x0 {} y0 {} x1 {} y1 {} x2 {} y2 {} x3 {} y3 {}".format(x0, y0, x1, y1, x2, y2, x3, y3))
...
def cal_rpn_target(labels, feature_map_shape, anchors, cls='Car', coordinate='lidar'):
    ...
    # warn("feature_map_shape: {}".format(feature_map_shape))
    w, l = feature_map_shape
    ...
    for batch_id in range(batch_size):
        ...
        index_x, index_y, index_z = np.unravel_index(id_neg, (w, l, 2))
